...
public class UtilsUi {
	...
	public static void imageLLViewReset(ImageView imageView, int bitmapW, int bitmapH, boolean isFull) {
		...
		HashMap<String, Integer> data = UtilsSystem.getDisplayMetrics();
		int width = data.get(UtilsSystem.systemWidth);// 320
		int height = data.get(UtilsSystem.systemHeight);// 480
		if (isFull) {
			if (width > height) {
	...
	public static void imageRLViewReset(ImageView imageView, int bitmapW, int bitmapH, boolean isFull) {
		...
		HashMap<String, Integer> data = UtilsSystem.getDisplayMetrics();
		int width = data.get(UtilsSystem.systemWidth);
		int height = data.get(UtilsSystem.systemHeight);
		if (isFull) {
			if (width > height) {
				layoutParams.width = (int) (bitmapW * 1.00f / bitmapH * height);
				layoutParams.height = height;
	...
	public static void resetRL(View... view) {
		float rote = UtilsSystem.getWidthRoate();
		if (view == null || rote == 1) {
			return;
		}
		for (View view2 : view) {
